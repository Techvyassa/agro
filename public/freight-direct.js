document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add a message about the direct API approach
    const apiNoticeContainer = document.createElement('div');
    apiNoticeContainer.className = 'alert alert-info';
    apiNoticeContainer.innerHTML = `
        <i class="fas fa-info-circle"></i> This tool calls the freight API directly. The request requires proper CORS configuration on the server.
    `;
    document.querySelector('#freightForm button[type="submit"]').parentNode.prepend(apiNoticeContainer);
    ...
    freightForm.addEventListener('submit', function(e) {
        ...
        // Direct API call (will require CORS to be configured on the target server)
        const apiUrl = 'https://agro-rpa.onrender.com/get-freight-estimates';
        
        // First try our proxy route
        fetch('/freight-proxy', {
            method: 'POST',
            headers: {
            ...
            resultsContainer.innerHTML = `
                <div class="col-12">
                    <div class="alert alert-danger">
                        ...
                        <div class="mt-3">
                            <p>Could not connect to the freight estimation API. Possible reasons:</p>
                            <ul>
                                <li>CORS restrictions on the API server</li>
