document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add the option to switch between demo and live API
    const apiSwitchContainer = document.createElement('div');
    apiSwitchContainer.className = 'mb-3 form-check ms-3';
    apiSwitchContainer.innerHTML = `
        <input type="checkbox" class="form-check-input" id="useMockData" checked>
        <label class="form-check-label" for="useMockData">Use demo data (avoid CORS issues)</label>
    `;
    document.querySelector('#freightForm button[type="submit"]').parentNode.prepend(apiSwitchContainer);
    ...
    freightForm.addEventListener('submit', function(e) {
        ...
        if (useMockData) {
            // Use the mock data approach with a slight delay to simulate network request
            setTimeout(() => {
                loader.style.display = 'none';
                displayResults(mockFreightData);
                console.log('Using mock data (API call simulated)')
                ...
                resultsContainer.innerHTML = `
                    <div class="col-12">
                        <div class="alert alert-danger">
                            ...
                            <div class="mt-3">
                                <p>The API cannot be accessed due to CORS or other restrictions. Try these options:</p>
                                <ol>
                                    <li>Check the "Use demo data" option above to see a demonstration with sample data</li>
                                    <li>Run this application with proper CORS configuration on the server</li>
