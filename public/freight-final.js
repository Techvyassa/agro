document.addEventListener('DOMContentLoaded', function() {
    ...
    // Mock data as a fallback when API is unavailable
    ...
    // Create a status box to show connectivity state
    const statusBox = document.createElement('div');
    statusBox.className = 'alert alert-warning mb-4';
    statusBox.id = 'apiStatus';
    resultsSection.prepend(statusBox);
    ...
    freightForm.addEventListener('submit', function(e) {
        ...
        statusBox.className = 'alert alert-warning mb-4';
        statusBox.innerHTML = '<i class="fas fa-sync fa-spin"></i> Attempting to connect to freight API...';
        ...
        // Try multiple approaches to get freight data
        // 1. Try the Laravel proxy route
        fetch('/freight-proxy', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            // Success! Real API data received
            loader.style.display = 'none';
            statusBox.className = 'alert alert-success mb-4';
            statusBox.innerHTML = '<i class="fas fa-check-circle"></i> Connected to live freight API. Showing real-time estimates.';
            displayResults(data);
        ...
        .catch(error => {
            ...
            // 2. Try direct API call as fallback
            fetch('https://agro-rpa.onrender.com/get-freight-estimates', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json'
            ...
            .then(data => {
                ...
                loader.style.display = 'none';
                statusBox.className = 'alert alert-success mb-4';
                statusBox.innerHTML = '<i class="fas fa-check-circle"></i> Connected directly to freight API. Showing real-time estimates.';
                displayResults(data);
            ...
            .catch(directError => {
                ...
                // 3. All API attempts failed, use mock data with a warning
                loader.style.display = 'none';
                statusBox.className = 'alert alert-warning mb-4';
                statusBox.innerHTML = `
                    <i class="fas fa-exclamation-triangle"></i> <strong>Using demonstration data</strong>
                    <p class="mb-0 mt-2">Could not connect to the freight API. Showing sample data for demonstration purposes.</p>
                    <p class="mb-0 small text-muted">Error details: ${error.message} | ${directError.message}</p>
                `;
                displayResults(mockFreightData);
                console.log('Using mock data due to API connection issues');
                console.log('Request payload that would be sent:', payload);
            });
        });
    });
